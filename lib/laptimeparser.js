...
function jsonObjMapper(jsonData) {
	let pageData = {};
	let motoData = jsonData.formImage.Pages;
...
function docHeaderParser(raceHeaders, index, pageData) {
	switch(index) {
		case 0:
			pageData.moto = raceHeaders.charAt(raceHeaders.length - 1);
			break;
		case 1:
		...
		case 2:
			...
			pageData.track = trackLocation[0].trim();
			pageData.location = trackLocation[1].trim();
			break;
		case 3:
			pageData.round = raceHeaders.charAt(7) === ' ' ? raceHeaders.substr(6, 1) : raceHeaders.substr(6, 2);
			pageData.date = raceHeaders.substr(raceHeaders.indexOf('-') + 2);
			break;
		case 4:
			pageData.class = raceHeaders;
			break;
		case 5:
